...
void PrecipitationEffect::setUpGeometries(unsigned int numParticles)
{
    ...
    if (!_quadStateSet)
    {
        ...
        _quadUniform = _quadStateSet->getOrCreateUniform("previousModelViewMatrix", osg::Uniform::FLOAT_MAT4);
        ...
        char fragmentShaderSource[] =
            ...
            "void main (void)\n"
            "{\n"
            "    color = colour * texture2D( baseTexture, texCoord);\n"
            "}\n";
    ...
    if (!_lineStateSet)
    {
        ...
        _lineUniform = _lineStateSet->getOrCreateUniform("previousModelViewMatrix",osg::Uniform::FLOAT_MAT4);
    ...
    if (!_pointStateSet)
    {
        ...
        //_pointStateSet->setMode(GL_BLEND, osg::StateAttribute::ON);
...
if (!_geometry) return;
    ...
    //osg::Matrix previous_modelview(renderInfo.getState()->getModelViewMatrix());
        ...
        ++itr)
    {
        ...
        if (_requiresPreviousMatrix)
        {
            ...
            //renderInfo.getState()->applyModelViewMatrix((*itr)->second.modelview);
            ...
            if (pitr != _previousCellMatrixMap.end())
            {
                ...
                //glLoadMatrix(pitr->second.modelview.ptr());
                //renderInfo.getState();
                //PrecipitationEffect::_lineUniform->set(pitr->second.modelview);
                //PrecipitationEffect::_quadUniform->set(pitr->second.modelview);
            }
            else
            {
                // use current modelview matrix as "previous" frame value, cancelling motion blurr effect
                //glMatrixMode(GL_TEXTURE);
                //glLoadMatrix((*itr)->second.modelview.ptr());
                //PrecipitationEffect::_lineUniform->set((*itr)->second.modelview);
                //PrecipitationEffect::_quadUniform->set((*itr)->second.modelview);
            }
        }
        else
        {
    ...
    if (needToApplyMatrix)
    {
        ...
        //renderInfo.getState()->applyModelViewMatrix(previous_modelview);
