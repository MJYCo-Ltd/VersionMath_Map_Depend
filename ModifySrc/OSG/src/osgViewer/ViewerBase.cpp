...
void ViewerBase::startThreading()
{
	...
	case(CullThreadPerCameraDrawThreadPerContext):
		...
		numThreadsOnEndBarrier = 1;
		break;
	case(CullThreadPerCameraDrawSingle):
		numThreadsOnStartBarrier = cameras.size() + 1;
		numThreadsOnEndBarrier = 1;
		break;
	default:
	...
	if (_threadingModel == CullDrawThreadPerContext)
	{
		...
		_threadingModel == CullThreadPerCameraDrawSingle)
	{
		_startRenderingBarrier = 0;
		_endCullBarrier = 0;
		_endRenderingDispatchBarrier = 0;
	...
	if (numThreadsOnStartBarrier > 1)
	{
		...
		if (CullThreadPerCameraDrawSingle == _threadingModel)
		{
			_endCullBarrier = new osg::BarrierOperation(numThreadsOnStartBarrier, osg::BarrierOperation::NO_OPERATION);
		}
	}
		...
		++citr)
	{
		...
		if (CullThreadPerCameraDrawSingle != _threadingModel)
		{
			...
			if (_startRenderingBarrier.valid() && _threadingModel == CullDrawThreadPerContext) gc->getGraphicsThread()->add(_startRenderingBarrier.get());

			/// Ôö¼Ó²Ã¼ôµÄ
			if (_endCullBarrier.valid() && _threadingModel == CullThreadPerCameraDrawSingle) gc->getGraphicsThread()->add(_endCullBarrier.get());
	...
	if ((_threadingModel == CullThreadPerCameraDrawThreadPerContext ||
		_threadingModel == CullThreadPerCameraDrawSingle) && numThreadsOnStartBarrier > 1)
	{
			...
			++camItr)
		{
			...
			if (_endCullBarrier.valid()) camera->getCameraThread()->add(_endCullBarrier.get());
...
void ViewerBase::renderingTraversals()
{
	...
	if (_endCullBarrier.valid()) _endCullBarrier->block();
	...
	double expendRenderingTraversals = endRenderingTraversals - beginRenderingTraversals;
	if (expendRenderingTraversals > 1)
	{
		OSG_WARN << "Rendering traversal time taken so long" << expendRenderingTraversals << std::endl;
	}
	...
	if (getViewerStats() && getViewerStats()->collectStats("update"))
	{
		...
		getViewerStats()->setAttribute(frameNumber, "Rendering traversals time taken", expendRenderingTraversals);
	}
