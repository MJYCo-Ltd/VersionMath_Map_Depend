...
void Renderer::cull()
{
    ...
    if (sceneView)
    {
        ...
        double beginCullTraversal = osg::Timer::instance()->delta_s(_startTick, beforeCullTick);
        double endCullTraversal = osg::Timer::instance()->delta_s(_startTick, afterCullTick);
        double expendCullTraversal = endCullTraversal - beginCullTraversal;
        if (expendCullTraversal > 1)
        {
            OSG_WARN << "Cull traversal time taken so long" << expendCullTraversal << std::endl;
        }
        ...
        if (stats && stats->collectStats("rendering"))
        {
            ...
            stats->setAttribute(frameNumber, "Cull traversal begin time", beginCullTraversal);
            stats->setAttribute(frameNumber, "Cull traversal end time", endCullTraversal);
            stats->setAttribute(frameNumber, "Cull traversal time taken", expendCullTraversal);
        }
...
void Renderer::draw()
{
    ...
    if (sceneView && !_done)
    {
        ...
        double beginDrawTraversal = osg::Timer::instance()->delta_s(_startTick, beforeDrawTick);
        double endDrawTraversal = osg::Timer::instance()->delta_s(_startTick, afterDrawTick);
        double expendDrawTraversal = endDrawTraversal - beginDrawTraversal;
        if (expendDrawTraversal > 1)
        {
            OSG_WARN << "Draw traversal time taken so long" << expendDrawTraversal << std::endl;
        }

        if (stats && stats->collectStats("rendering"))
        {
            stats->setAttribute(frameNumber, "Draw traversal begin time", beginDrawTraversal);
            stats->setAttribute(frameNumber, "Draw traversal end time", endDrawTraversal);
            stats->setAttribute(frameNumber, "Draw traversal time taken", expendDrawTraversal);
        }
