    ...
    bool considerUpsampling) const
{
    ...
    const DataExtent& dataExtent = getDataExtentsUnion();
    GeoExtent keyExtent = key.getExtent();
    if (!dataExtent.getSRS()->isHorizEquivalentTo(keyExtent.getSRS()))
    {
        keyExtent = keyExtent.transform(dataExtent.getSRS());
    }
    // Reject if the extents don't overlap at all.
    // (Note: this does not consider min/max levels, only spatial extents)
    if (!dataExtent.intersects(keyExtent,false))
    {
        return TileKey::INVALID;
    }
