    ...
    std::vector< osg::ref_ptr<osg::Shader> >& out_shaders) const
{
    ...
    if ( hasGS )
    {
        ...
        buf << "\nvoid VP_LoadVertex(const in int index) \n"
            << "{ \n";

        // Copy input block to stage globals:
        buf << INDENT << "switch(index)\n";
        buf << INDENT << "{\n";
	buf << INDENT << "case 0:\n";        
	for(Variables::const_iterator i = vars.begin(); i != vars.end(); ++i)
            buf << INDENT << INDENT << i->name << " = vp_in[0]." << i->name << "; \n";
	buf << INDENT << "break;\n";
	buf << INDENT << "case 1:\n";        
	for(Variables::const_iterator i = vars.begin(); i != vars.end(); ++i)
            buf << INDENT << INDENT << i->name << " = vp_in[1]." << i->name << "; \n";
	buf << INDENT << "break;\n";
	/*buf << INDENT << "case 2:\n";        
	for(Variables::const_iterator i = vars.begin(); i != vars.end(); ++i)
            buf << INDENT << INDENT << i->name << " = vp_in[2]." << i->name << "; \n";
	buf << INDENT << "break;\n";
	buf << INDENT << "case 3:\n";        
	for(Variables::const_iterator i = vars.begin(); i != vars.end(); ++i)
            buf << INDENT << INDENT << i->name << " = vp_in[3]." << i->name << "; \n";
	buf << INDENT << "break;\n";
	buf << INDENT << "case 4:\n";        
	for(Variables::const_iterator i = vars.begin(); i != vars.end(); ++i)
            buf << INDENT << INDENT << i->name << " = vp_in[4]." << i->name << "; \n";
	buf << INDENT << "break;\n";
	buf << INDENT << "case 5:\n";        
	for(Variables::const_iterator i = vars.begin(); i != vars.end(); ++i)
            buf << INDENT << INDENT << i->name << " = vp_in[5]." << i->name << "; \n";
	buf << INDENT << "break;\n";*/
        buf << INDENT << "}\n";
        buf << "} \n";
